...
# STEP 1: GET A REQUEST TOKEN (the application contacts twitter, and requests a token--> set up initial connection between application and Twitter)
...
#above apart of requests_oauthlib--> create oauthsession where we privide client key and secret
fetch_response = oauth.fetch_request_token(request_token_url)
resource_owner_key = fetch_response.get('oauth_token')
resource_owner_secret = fetch_response.get('oauth_token_secret')
#get back an owner key and a owner secret; tokens that will allow us to register a particular user
...
#send them over to twitter, generate special token and send special token back to the application so we can use it
...
#print out url telling the user to go over there and authorize it (usually wouldnt have them copy and paste, will open the browser to the page)
verifier = input('Paste the verification code here: ')
#paste URL and they will get back this verifier
...
oauth = OAuth1Session(client_key,
                          ...
                          verifier=verifier)
#above things we got from step 0, 1, 2, 3
oauth_tokens = oauth.fetch_access_token(access_token_url)

resource_owner_key = oauth_tokens.get('oauth_token')
resource_owner_secret = oauth_tokens.get('oauth_token_secret')

print(resource_owner_key, resource_owner_secret) #print so we can see what they are
...
#below is what we do to finally go and get our data
